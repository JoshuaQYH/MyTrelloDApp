...
class setVote extends React.Component {
  ...
  Vote = async(taskAddr, voteFlag)=> {

    try{
      let _web3 = this.state.web3;
      let _account = this.state.accounts[0];
      console.log(taskAddr)
      console.log(voteFlag)
      const taskContract = new _web3.eth.Contract(Task.abi, taskAddr);
      console.log(taskContract)
      if(voteFlag){
        // 可投票
        await taskContract.methods
        .setVoteFlag(true).send({from: _account, gas: "4700000"})
        .then(function(result){
          console.log(result);
        })
        alert("You can vote in the task.")
      }
      else{
        // 不可投票
        await taskContract.methods
        .setVoteFlag(false).send({from: _account, gas: "4700000"})
        .then(function(result){
          console.log(result);
        })
        alert("You can't vote int the task.")
      }
    }
    catch(error){
      console.error(error);
      alert("there's something error. You can open the console for detail.")
    }
  }
        ...
        <div className={classes.root}>
          ...
          <div className={classes.appContent}>
            ...
            <main className={classes.mainContent}>
                <SetVote transferMsg = {msg => this.transferMsg(msg)} />
            </main>
          </div>
        </div>
      </MuiThemeProvider>
